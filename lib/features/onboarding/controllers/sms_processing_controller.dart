...
class SmsProcessingController extends GetxController {
  ...
  Future<void> processSmsMessages() async {
    ...
    try {
      ...
      // Save to database
      final dbTransactions = processedTransactions
          .map(SmsTransactionModel.toTransactionModel)
          .toList();
      await TransactionRepository.instance.saveTransactions(dbTransactions);

      progress.value = 0.9;
      transactions.assignAll(processedTransactions);

      // Mark processing complete and start listener
      await SettingsRepository.instance.setSmsProcessingCompleted();
