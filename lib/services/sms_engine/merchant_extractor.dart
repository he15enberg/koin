...
const Set<String> _kGarbageMerchantTokens = {
  'debit',
  'credit',
  'ref',
  'rs',
...
class MerchantExtractor {
  static String extractMerchantName(
    ...
    for (final extractor in extractors) {
      ...
      if (merchantName.isNotEmpty &&
          !_kGarbageMerchantTokens.contains(merchantName.trim().toLowerCase())) {
  ...
  static String _extractFromUpiPattern(
    ...
    // to the remaining extractors instead of naming the merchant after the
    // transaction type itself (that used to produce merchant names like
    // "Debit"/"Credit", which are meaningless and uncategorizable).
    return '';
  }
  ...
  static String _extractFromAtToPattern(
    ...
    // "at"/"to" are word-boundary-anchored -- without it this used to match
    // "to" embedded at the tail of an unrelated word (e.g. "ZOMATO order"
    // matched the "TO" inside "ZOMATO" itself and extracted "order").
